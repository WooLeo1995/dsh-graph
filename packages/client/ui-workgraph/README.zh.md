...
- `isGraphStartChange` 识别图的唯一开始：set 提交恰好携带一条 `created` 历史。会话引擎要求每个上下文恰好一个 `start` 匹配，因此整个生命周期折叠为每个图身份一个节点。
...
键控 chat 渲染器（`WorkGraphNode.tsx`）：逐层节点卡带账本点状态（`done`/`ongoing`/`warning`/`error`）、未达成依赖的等待说明、失败/受阻卡上的失败来源、已达成轮次徽章、终节点双线边框并标注、头部（目标、状态胶囊、计划版本、消耗与预算、发现、暂停原因）、图例与可选详情面板（目标、规格、轮次、依赖、发现来源、失败）。主题 token 驱动全部颜色。

## 组合

生产者注入 `conversationEvents`、`slots` 与 `locale`。自定义应用挂载其属主与本插件；视图只读，仅凭会话事件即可工作（无需调度器 provider）：
